...
//			m_hEvent.Reset();
			...
			boost::unique_lock<boost::mutex> eventLock(m_mtxEvent);
			if (m_hEvent.wait_for(eventLock, boost::chrono::milliseconds(iTimeoutInMilliSeconds)) == std::cv_status::timeout) {
				m_section.lock();
				return MSGQ_TIMEOUT;
			}
