...
bool AppliactionDesc::IsCurrentThread() {
	return boost::this_thread::get_id() == TVPMainThreadID;
}
...
//	CLog::Log(LOGDEBUG, "CRenderManager::Configure - change configuration. %dx%d. display: %dx%d. framerate: %4.2f. format: %s", picture.iWidth, picture.iHeight, picture.iDisplayWidth, picture.iDisplayHeight, fps, formatstr.c_str());
	...
	if (m_stateEvent.wait_for(stateLock, boost::chrono::milliseconds(1000)) == std::cv_status::timeout)
	{
...
void CRenderManager::FrameWait(int ms)
{
	...
	while (m_presentstep == PRESENT_IDLE && !timeout.IsTimePast())
		m_presentevent.wait_for(lock, boost::chrono::milliseconds(timeout.MillisLeft()));
}
...
//	m.presentfield = sync;
	...
	if (wait)
	{
		...
		{
			m_presentevent.wait_for(lock, boost::chrono::milliseconds(20));
...
int CRenderManager::WaitForBuffer(volatile std::atomic_bool&bStop, int timeout)
{
	...
	if (!m_bRenderGUI /*|| !g_application.GetRenderGUI()*/)
	{
		...
		m_presentevent.wait_for(lock, boost::chrono::milliseconds(sleeptime));
		DiscardBuffer();
		return 0;
	}
	...
	while (m_free.empty())
	{
		m_presentevent.wait_for(lock, boost::chrono::milliseconds(std::min(50, timeout)));
		if (endtime.IsTimePast() || bStop)
		{
			if (timeout != 0 && !bStop)
			{
