...
#if defined(ANDROID)
		__android_log_print(ANDROID_LOG_ERROR, "tjsInterCodeExec.cpp", "%s", "*** *** tTJSVariantArrayStack not release");
	...
	tjs_int exobjreg)
{
	...
	try
	{
		...
		catch(...)
		{
			...
			throw;
#elif defined(_MSC_VER)	
			TVPOutputDebugString("================> ExecuteCodeInTryBlock throw\n");
			//__debugbreak(); throw;
			throw;
#else
			throw;
...
#define TJS_END_FUNC_CALL_ARGS                                            \
	...
	{                                                                     \
		if(alloc_args) delete [] pass_args;                               \
		if(pass_args_v) delete [] pass_args_v;                            \
		/*__debugbreak();*/ TVPOutputDebugString("========>TJS_END_FUNC_CALL_ARGS"); throw; \
	}                                                                     \
	if(alloc_args) delete [] pass_args;                                   \
	if(pass_args_v) delete [] pass_args_v;
	...
	TJS_BEGIN_FUNC_CALL_ARGS(code + 4)
		...
		else
		{
			...
			catch(...)
			{
				...
				throw;
#elif defined(_MSC_VER)	
				TVPOutputDebugString("================> TJSDefaultFuncCall throw\n");
				throw;
#else
				throw;
#endif	
