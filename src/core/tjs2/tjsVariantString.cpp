...
static tTJSSpinLock *pTJSStringHeapCS = NULL;
...
#define TJS_CHECK_FREE_BLOCK_MAX 50
	...
	{	// thread-protected
		if (pTJSStringHeapCS == NULL) {
			pTJSStringHeapCS = new tTJSSpinLock();
		}
		tTJSSpinLockHolder csh(*pTJSStringHeapCS);
...
{
#if defined(ANDROID)
	__android_log_print(ANDROID_LOG_ERROR, "tjsVariantString.cpp", "%s", "*** *** tTJSVariantString_S");
#endif
...
{
#if defined(ANDROID)
	__android_log_print(ANDROID_LOG_ERROR, "tjsVariantString.cpp", "%s", "*** *** tTJSVariantString_S 2");
#endif
	pthread_mutex_init(&RefCount_lock, NULL);
...
#if defined(ANDROID)
	__android_log_print(ANDROID_LOG_ERROR, "tjsVariantString.cpp", "%s", "*** *** tTJSVariantString_S 3");
#endif
	pthread_mutex_init(&RefCount_lock, NULL);
	RefCount_pth = c.RefCount_pth;
	LongString = c.LongString;
