...
#else
	pthread_mutex_t atom_lock_spin;
	virtual ~tTJSSpinLock() { pthread_mutex_destroy(&atom_lock_spin); }
#endif	
#endif
	tTJSSpinLock();
//public:
//	tTJSSpinLock() {
//#if defined(ANDROID)
//		__android_log_print(ANDROID_LOG_ERROR, "tjsUtils.h", "%s", "*** *** tTJSSpinLock");
//#endif
...
class tRefHolder
{
	struct _RefCountWrapper
	{
		...
		_RefCountWrapper() :_ptr(nullptr), RefCount(1) {}
		_RefCountWrapper(T* p) : _ptr(p), RefCount(1) {}
