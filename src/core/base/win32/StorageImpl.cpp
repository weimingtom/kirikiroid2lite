...
//https://www.cnblogs.com/highway-9/p/5757058.html
...
void TVPListDir(const std::string &folder, std::function<void(const std::string&, int)> cb) {
	...
	//__debugbreak(); throw;
#if 1
	OutputDebugString(L"=========================>void TVPListDir(\n");
	...
	folder2 += "*.*";
	WIN32_FIND_DATA ffd;
	HANDLE handle = ::FindFirstFile(UTF82WCS((folder2).c_str()).c_str(), &ffd);
	if (handle != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			std::string fullpath = folder + "/" + WCS2UTF8(ffd.cFileName).c_str();
			if (!TVP_stat(fullpath.c_str(), stat_buf))
				continue;
			/*
			int arg = 0;
			...
			cb(WCS2UTF8(ffd.cFileName).c_str(), stat_buf.st_mode);
...
void TVPGetLocalFileListAt(const ttstr &name, const std::function<void(const ttstr&, tTVPLocalFileInfo*)>& cb) {
...
#else
	//__debugbreak(); throw;
	OutputDebugString(L"=========================>void TVPGetLocalFileListAt(\n");
	std::string folder(name.AsNarrowStdString());
	std::string folder2 = folder;
	if (!endsWith(folder, "/"))
	{
	...
	if (handle != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			...
			if (!TVP_stat(fullpath.c_str(), stat_buf))
				continue;
			ttstr file(WCS2UTF8(ffd.cFileName).c_str());
			if (file.length() <= 2) {
				if (file == TJS_W(".") || file == TJS_W(".."))
					continue;
			}
...
#if defined(_MSC_VER)
		char kkk[1024] = { 0 };
		sprintf(kkk, "==============>TVPOpenArchive: %d\n", i);
		OutputDebugStringA(kkk);
#elif defined(ANDROID)
		LOGE("==============>TVPOpenArchive: %d\n", i);
#endif
