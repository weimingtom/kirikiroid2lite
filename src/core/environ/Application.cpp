...
{
#if defined(ANDROID)
	__android_log_print(ANDROID_LOG_ERROR, "tTVPApplication", "%s", "*** *** tTVPApplication");
#endif
...
// 	windows_list_.clear();
	...
	pthread_mutex_destroy(&m_msgQueueLock);
}
#if 0
struct SEHException {
	unsigned int Code;
...
//	TVPRestoreFullScreenWindowAtActivation();
	...
	for (auto p_it = m_activeEvents.begin(); p_it != m_activeEvents.end(); ++p_it)
	{
		const auto &it = *p_it;
