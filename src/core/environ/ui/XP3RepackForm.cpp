...
static void WalkDir(const ttstr &dir, const std::function<void(const ttstr&, tjs_uint64)>& cb) {
	...
	for (auto& p_it = files.begin(); p_it != files.end(); ++p_it)
	{
		auto& it = *p_it;
		cb(prefix + it.first, it.second);
	}
	//for (ttstr &path : subdirs) {
	for (auto& p_path = subdirs.begin(); p_path != subdirs.end(); ++p_path)
	{
		auto& path = *p_path;
...
public:
	TVPXP3Repacker(const std::string &rootdir) : RootDir(rootdir), UpdateMS(100), ProgressForm(nullptr), LastUpdate(0) {}
	~TVPXP3Repacker();
...
public:
	TVPXP3RepackFileListForm() : m_pRepacker(nullptr) {}
