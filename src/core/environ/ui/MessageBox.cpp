...
void TVPMessageBoxForm::init(const std::string &caption, const std::string &text, int nBtns, const std::string *btnText, const std::function<void(int)> &callback)
{
	...
	//for (Widget *btn : btns) {
	for (auto p_btn = btns.begin(); p_btn != btns.end(); ++p_btn)
	{
		const auto& btn = *p_btn; 
...
{
	_title = dynamic_cast<Text*>(allNodes.findController<cocos2d::Node>("title"));
	_textContent = dynamic_cast<Text*>(allNodes.findController<cocos2d::Node>("content"));
	_textContainer = dynamic_cast<ScrollView*>(allNodes.findController<cocos2d::Node>("text"));

	_btnBody = dynamic_cast<Button*>(allNodes.findController<cocos2d::Node>("btnBody"));
...
void TVPSimpleProgressForm::bindBodyController(const NodeMap &allNodes)
{
	...
	_btnContainer = allNodes.findController<cocos2d::Node>("btnList");
