...
//see https://gitee.com/qihuancode/imgui_easy_pen/blob/main/main.cpp
...
bool FileUtilsLinux_mod::init()
{
    ...
    if (pos >= 0) {
        _writablePath += appPath.substr(pos);
    }
    _writablePath += "/";
...
//for apple
#if defined(__APPLE__)
FileUtilsApple_mod::FileUtilsApple_mod()
{}

bool FileUtilsApple_mod::init()
{
CCLOG("<<<<<<<<<<FileUtilsApple_mod::init");
...
string FileUtilsApple_mod::getWritablePath() const
{
    struct stat st;
    stat(_writablePath.c_str(), &st);
    if (!S_ISDIR(st.st_mode)) {
...
bool FileUtilsApple_mod::isFileExistInternal(const std::string& strFilePath) const
{
    if (strFilePath.empty())
    {
        return false;
