...
struct XP3FilterDecoder {
	...
	XP3FilterDecoder() : ScriptEngine(new tTJS()), ManagedDecoder(nullptr), ManagedFilter(nullptr) { }
	~XP3FilterDecoder() {
...
// static boost::mutex sTVPScriptEngineStackLock;
...
#if defined(ANDROID)
		__android_log_print(ANDROID_LOG_ERROR, "xp3filter.cpp", "%s", "*** *** FetchXP3Decoder");
